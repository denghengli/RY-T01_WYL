...
void press_ao_comp_enter(void *para)
{
    ...
    right_pos = 0;
    integer = (int)fabs(g_SysData.Data.Para.pressAOOffset);
    dec = (int)(fabs(g_SysData.Data.Para.pressAOOffset) * 100) % 100;
    //符号位
    if (g_SysData.Data.Para.pressAOOffset >= 0) ao_comp_char[0] = 0;
...
{
    ui_cur_state = PARA_SET_THR_SELECT_1;
    para_set_thr_select_1(NULL);
}
...
void press_ao_comp_ok(void *para)
{
    ...
    g_SysData.Data.Para.pressAOOffset = ao_comp_char[0] == 1 ? -value : value;
