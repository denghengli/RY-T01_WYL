...
void tem_ao_comp_enter(void *para)
{
    ...
    right_pos = 0;
    integer = (int)fabs(g_SysData.Data.Para.temAOOffset);
    dec = (int)(fabs(g_SysData.Data.Para.temAOOffset) * 100) % 100;
    //符号位
    if (g_SysData.Data.Para.temAOOffset >= 0) ao_comp_char[0] = 0;
    else ao_comp_char[0] = 1;
    ao_comp_char[1] = integer % 10;
    ao_comp_char[2] = dec / 10;
    ao_comp_char[3] = dec % 10;

    integer = (int)g_SysData.Data.Para.temAOK;
    dec = (int)(g_SysData.Data.Para.temAOK * 100) % 100;
    ao_comp_char[4] = integer % 10;
    ao_comp_char[5] = dec / 10;
    ao_comp_char[6] = dec % 10;
    
    ui_cur_state = TEM_AO_COMP;
}
...
{
    ui_cur_state = PARA_SET_TWO_SELECT_5;
    para_set_two_select_5(NULL);
}
...
void tem_ao_comp_ok(void *para)
{
    ...
    g_SysData.Data.Para.temAOOffset = ao_comp_char[0] == 1 ? -value : value;

    integer = ao_comp_char[4];
    dec = ao_comp_char[5] * 10 + ao_comp_char[6];
    value = integer + (float)(dec / 100.0 + 0.001);
    g_SysData.Data.Para.temAOK = value;
    ParaData_Save(0); 
