...
void humit_ao_comp_enter(void *para)
{
    ...
    right_pos = 0;
    integer = (int)fabs(g_SysData.Data.Para.humitAOOffset);
    dec = (int)(fabs(g_SysData.Data.Para.humitAOOffset) * 100) % 100;
    //符号位
    if (g_SysData.Data.Para.humitAOOffset >= 0) ao_comp_char[0] = 0;
    else ao_comp_char[0] = 1;
    ao_comp_char[1] = integer % 10;
    ao_comp_char[2] = dec / 10;
    ao_comp_char[3] = dec % 10;

    integer = (int)g_SysData.Data.Para.humitAOK;
    dec = (int)(g_SysData.Data.Para.humitAOK * 100) % 100;
    ao_comp_char[4] = integer % 10;
    ao_comp_char[5] = dec / 10;
    ao_comp_char[6] = dec % 10;
    
    ui_cur_state = HUMIT_AO_COMP;
}
...
{
    ui_cur_state = PARA_SET_THR_SELECT_3;
    para_set_thr_select_3(NULL);
}
...
void humit_ao_comp_ok(void *para)
{
    ...
    g_SysData.Data.Para.humitAOOffset = ao_comp_char[0] == 1 ? -value : value;

    integer = ao_comp_char[4];
    dec = ao_comp_char[5] * 10 + ao_comp_char[6];
    value = integer + (float)(dec / 100.0 + 0.001);
    g_SysData.Data.Para.humitAOK = value;
    ParaData_Save(0); 
