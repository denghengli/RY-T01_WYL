...
static char section_area_char[4]; //两位整数+两位小数
...
void section_area_set_enter(void *para)
{
    ...
    dec = (int)(g_SysData.Data.Para.sectionArea * 100) % 100;
    
    section_area_char[0] = integer / 10;
    section_area_char[1] = integer % 10;
    section_area_char[2] = dec / 10;
    section_area_char[3] = dec % 10;
...
void section_area_set(void *para)
{
    ...
    if (para_value == 1) //立刻显示
    {
        ...
        snprintf(tmp_str, sizeof(tmp_str), "%d%d.%d%d", section_area_char[0], section_area_char[1], section_area_char[2], section_area_char[3]);
    }
    else if (para_value == 2) //立刻隐藏
    {
        hold_delay_cnt = 10; //持续一会之后再闪烁
        
        if (right_pos == 0) snprintf(tmp_str, sizeof(tmp_str), " %d.%d%d", section_area_char[1], section_area_char[2], section_area_char[3]);
        else if (right_pos == 1) snprintf(tmp_str, sizeof(tmp_str), "%d .%d%d", section_area_char[0], section_area_char[2], section_area_char[3]);
        else if (right_pos == 2) snprintf(tmp_str, sizeof(tmp_str), "%d%d. %d", section_area_char[0], section_area_char[1], section_area_char[3]);
        else if (right_pos == 3) snprintf(tmp_str, sizeof(tmp_str), "%d%d.%d ", section_area_char[0], section_area_char[1], section_area_char[2]);
    }
    else //闪烁
    {
        if (hold_delay_cnt) 
...
void section_area_set_ok(void *para)
{
    ...
    dec = section_area_char[2] * 10 + section_area_char[3];
